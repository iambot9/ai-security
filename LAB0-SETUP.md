...
```
LAB 0  →  LAB 1  →  LAB 2  →  LAB 3A–3B  →  LAB 4  →  LAB 5  →  LAB 6
Setup     Build     Break     Defend         Agentic   Gateway   Deploy
          Agent     Agent     (custom+ent)   Security  (Azure)   (future)
```
...
| Lab | Title | Phase | Focus |
|-----|-------|-------|-------|
| LAB 0 | Environment Setup | Setup | Python, FastAPI, project scaffold |
| LAB 1 | Build the Vulnerable Agent | Build | Intentionally insecure AI chatbot |
| LAB 2 | Attack the Agent | Break | Prompt injection, data disclosure, SSRF, SQLi, path traversal |
| LAB 3A | Custom Defenses | Defend (hand-built) | Input/output guards, RBAC, rate limiting, audit logging |
| LAB 3B | Enterprise Defense Tools | Defend (third-party) | LLM Guard, Presidio, Guardrails AI |
| LAB 4 | Agentic Security | Defend (advanced) | Tool interception, trust boundaries, confused deputy |
| LAB 5 | AI Gateway | Govern | Centralized LLM traffic control, Azure APIM GenAI Gateway |
| LAB 6 | Azure Deployment | Deploy (future) | Cloud deployment reference architecture |

### 6.2 Phase-by-Phase Description

**Phase 1 — Build (LAB 1)**

You will construct a realistic AI agent chatbot using FastAPI and a mock LLM engine. The agent has access to tools: it can query a SQLite database containing simulated user records and financial data, read files, and make outbound HTTP requests. The agent is intentionally built with security mistakes — verbose error messages, no input validation, overly permissive tool access, secrets in the system prompt — because you need a realistic target before you can practice attacking and defending.
...
You will attack the agent you built using techniques mapped to the OWASP Top 10 for LLM Applications. In a single comprehensive lab, you will execute prompt injection (direct, indirect, payload splitting), sensitive data disclosure (system prompt extraction, PII harvesting, credential leakage), excessive agency exploitation (unauthorized modifications, SSRF, path traversal), and RAG poisoning via document upload.
...
**Phase 3A — Defend with Custom Code (LAB 3A)**
...
Defenses you will build: input validation (regex, keyword matching), output filtering (PII redaction), RBAC (tool-level permissions by role), rate limiting, prompt armor (boundary markers, canary tokens), and audit logging.
...
**Phase 3B — Defend with Enterprise Tools (LAB 3B)**

You will replace or augment your custom defenses with three enterprise-grade AI security libraries: LLM Guard (ML-based injection detection), Microsoft Presidio (NER-based PII anonymization), and Guardrails AI (output governance). A comparison harness will run the same attacks through custom vs. enterprise defenses to measure detection rates and trade-offs.
...
**Phase 4 — Agentic Security (LAB 4)**

You will address threats specific to agentic AI architectures: multi-step tool chaining attacks, confused deputy problems, delegation and escalation patterns, and trust boundary enforcement. You will build tool call interception middleware, implement human-in-the-loop controls for sensitive operations, and design multi-agent trust boundaries.

The learning outcome: you will understand why agentic AI introduces risks beyond what the standard OWASP LLM Top 10 covers, and how to apply IAM principles to agent-to-agent delegation.

**Phase 5 — AI Gateway (LAB 5)**

You will build a centralized LLM traffic control layer — an AI Gateway — that sits in front of your agent and enforces organization-wide policies. You will first build a local reverse proxy to understand the pattern, then map each capability to Azure API Management's GenAI Gateway features: token-based rate limiting, content filtering via Azure AI Content Safety, model routing and load balancing, cost tracking per consumer, and semantic caching.

The learning outcome: you will understand the AI Gateway pattern as an enterprise control plane and be able to architect a centralized LLM governance layer using Azure APIM.

**Phase 6 — Azure Deployment (LAB 6, future reference)**

A reference architecture for deploying the full lab stack to Azure: Azure Container Apps, Azure AI Content Safety, Azure API Management as AI Gateway, Azure Monitor with Log Analytics, Entra ID for authentication, and Key Vault for secrets management.
...
|---------------------|--------|
| LLM01: Prompt Injection | LAB 2 (attack), LAB 3A (defend) |
| LLM02: Insecure Output Handling | LAB 2 (attack), LAB 3A–3B (defend) |
| LLM03: Training Data Poisoning / RAG Poisoning | LAB 2 (attack) |
| LLM04: Model Denial of Service | LAB 2 (attack), LAB 5 (gateway rate limiting) |
| LLM06: Sensitive Information Disclosure | LAB 2 (attack), LAB 3A–3B (defend) |
| LLM07: Insecure Plugin Design | LAB 4 (agentic security) |
| LLM08: Excessive Agency | LAB 2 (attack), LAB 4 (agentic security) |
| LLM10: Unbounded Consumption | LAB 5 (gateway controls) |
...
Each lab has its own markdown file in the `ai-security-lab/` directory. Each lab assumes you have completed all previous labs. The attack scripts in `attacks/` are designed to be run against the agent as-built at that point in the series.
...
*AI Security Lab Series — LAB 0 of 7*
