...
LAB 2 will attack this agent. LABs 3A–3B will harden it with custom and enterprise defenses. LAB 4 will address agentic-specific threats. LAB 5 will add an AI Gateway layer. **Do not skip building it carefully — understanding the code is what makes the attacks meaningful.**
...
**LAB 2 — Attack the Agent**: You will use curl, Python scripts, and prompt engineering to extract the system prompt via prompt injection, enumerate customer SSNs via SQL injection, exfiltrate API keys, traverse file paths, probe internal networks via SSRF, gain admin access, and poison the RAG pipeline.

**LAB 3A — Custom Defenses**: You will build input guards, output filters, RBAC, rate limiting, prompt armor, and audit logging from scratch.

**LAB 3B — Enterprise Defense Tools**: You will integrate LLM Guard, Microsoft Presidio, and Guardrails AI, then compare them against your custom defenses.

**LAB 4 — Agentic Security**: You will address multi-step tool chaining, confused deputy attacks, and trust boundary enforcement.

**LAB 5 — AI Gateway**: You will build a centralized LLM traffic control layer and map it to Azure API Management's GenAI Gateway capabilities.

---

*AI Security Lab — LAB 1 of 7*
